...
- `end_of_conversation: true` only when user signals completion
...
### Vercel (Frontend)
1. Set `VITE_API_URL=https://your-render-url.onrender.com` in Vercel environment variables
...
|---|---|---|
| `GROQ_API_KEY` | Yes | Groq API key (backend) — free at console.groq.com |
| `VITE_API_URL` | No | Backend URL (defaults to `/api` proxy in dev) |
